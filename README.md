...
Each identifer must start with a _ID_Start_ and continue with a _ID_Continue_
...
ranges. All the UCD ID_Start and XID_Start properties incorrectly list
them there, and not in X?ID_Continue btw.
...
for that use-case. (_Maybe still allow that for conformance testing_.
This is the recommended profile, don't fall into the unicode identifier trap.
_E.g. zig was right, rejecting those proposals._)
...
`mkscripts.pl` from the current UCD, sorted into _Recommended Scripts_
(sorted by codepoints), _Not Recommended Scripts_, i.e. _Excluded
Scripts_ (sorted alphabetically) and _Limited Use Scripts_ (sorted by
codepoint).
...
- NFC lookup optimization:
  ...
  NFC + PROFILE_4 + CHECK_XID with the script byte in the first decompose lookup
  `UN8IF_canon_tbl`.
