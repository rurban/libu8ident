...
static void emit_nsm_letters(FILE *f) {
  FILE *mf = fopen("mark.h", "r");
  if (!mf) {
    fprintf(stderr, "Cannot open mark.h\n");
    return;
  }
  char line[4096];
  bool copying = false;
  while (fgets(line, sizeof(line), mf)) {
    if (!copying && strstr(line, "struct nsm_ws {"))
      copying = true;
    if (copying) {
      fputs(line, f);
      if (strstr(line, "};")) {
  ...
  // Now copy nsm_letters array
  copying = false;
  while (fgets(line, sizeof(line), mf)) {
    if (!copying && strstr(line, "const struct nsm_ws nsm_letters[] = {"))
      copying = true;
    if (copying) {
      fputs(line, f);
      if (strstr(line, "};")) {
...
static void gen_unitr39(void) {
  ...
  // Emit struct range_bool
  fputs(
      "struct range_bool { uint32_t from; uint32_t to; };\n"
      "\n",
      f);

  // Emit struct sc (needed by nonxid_script_list search)
  fputs(
      "struct sc {\n"
      "  uint32_t from;\n"
      "  uint32_t to;\n"
  ...
  fputs(
      ...
      "    // clang-format off\n",
      f);
  for (size_t i = 0; i < ARRAY_SIZE(bidi_list); i++) {
    fprintf(f, "    {0x%X, 0x%X},\n", bidi_list[i].from, bidi_list[i].to);
  }
  ...
  // Emit greek_confus_list
  fputs("static const uint32_t greek_confus_list[] = {\n"
        "    // clang-format off\n",
        f);
  for (size_t i = 0; i < ARRAY_SIZE(greek_confus_list); i++) {
    fprintf(f, "    0x%X,\n", greek_confus_list[i]);
  }
  fputs("    // clang-format on\n"
        "};\n"
  ...
  // Emit nsm_letters from mark.h
  emit_nsm_letters(f);
  fputs("\n", f);

  fputs("#endif /* !HAVE_CONFIG_H */\n"
        "\n",
        f);
