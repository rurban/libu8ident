...
# search the singles. Try this here.
# icu does 2-or 3-way tables as with our norm headers, and special tries.
...
open my $H, ">", "scripts1.h" or die "writing scripts1.h $!";
print $H <<"EOF";
...
/* libu8ident - Check unicode security guidelines for identifiers.
   ...
   generated by mkscripts1.pl, do not modify.
   UNICODE version $ucd_version[0].$ucd_version[1]
   seperate ranges from singles.
*/
...
// all XID's properly.
#  ifndef EXT_SCRIPTS
const struct sc xid_script_list_2[] = {
    // clang-format off
EOF
my ($b, $s);
for my $r (@SCR) {
...
const struct sc1 xid_script_list_1[] = {
    // clang-format off
EOF
for my $r (@SCR) {
  if ($r->[0] == $r->[1]) {
    printf $H "    {0x%04X, %d},\t// %s", $r->[0], $SC{$r->[2]}, $r->[2];
    if (@$r == 4) {
      printf $H ", originally SC %s\n", $r->[3];
    } else {
      printf $H "\n";
...
#  else
extern const struct sc xid_script_list_2[%u]; // ranges
extern const struct sc1 xid_script_list_1[%u]; // singles
#  endif
...
// codepoints.
#ifndef EXT_SCRIPTS
const struct sc nonxid_script_list_2[] = {
    // clang-format off
EOF
($b, $s) = (0, 0);
for my $r (@SCRF) {
...
const struct sc1 nonxid_script_list_1[] = {
    // clang-format off
EOF
for my $r (@SCRF) {
  if ($r->[0] == $r->[1]) {
...
#else
extern const struct sc nonxid_script_list_2[%u]; // ranges
extern const struct sc1 nonxid_script_list_1[%u]; // singles
#endif
...
for my $r (@SCXR) {
  ...
  if ($r->[0] == $r->[1]) {
    ...
    if (@list == 1) {
      $s--;
      printf $H "    // {0x%04X, \"%s\"},\t// %s, moved to sc proper\n", $r->[0], $code, $r->[2];
    } else {
      printf $H "    {0x%04X, \"%s\"},\t// %s\n", $r->[0], $code, $r->[2];
    }
  }
...
#else
extern const struct scx scx_list_2[%u];
extern const struct scx1 scx_list_1[%u];
#endif
...
#  ifndef EXT_SCRIPTS
const struct range_bool allowed_id_list_2[] = {
    // clang-format off
EOF
($b, $s) = (0, 0);
for my $r (@ALLOWED) {
...
const uint32_t allowed_id_list_1[] = {
    // clang-format off
EOF
for my $r (@ALLOWED) {
  if ($r->[0] == $r->[1]) {
...
#  else
extern const struct range_bool allowed_id_list_2[%u];
extern const uint32_t          allowed_id_list_1[%u];
#  endif
...
for my $name (@NORM_QC) {
  ...
  my $list = $NORM . "_" . $M;
  no strict 'refs';
  printf $H <<'EOF', $NORM, $maybe, $NORM, $list, $list;

// %s_Quick_Check=%s
#  if !defined U8ID_NORM || U8ID_NORM == %s
#    ifndef EXT_SCRIPTS
const struct range_bool %s_list_2[] = {
    // clang-format off
EOF
  ($b, $s) = (0, 0);
  for my $r (@{$name}) {
...
EOF
  ...
  printf $H <<'EOF', $list, $b, $list, $s;
    // clang-format on
};
#    else
extern const struct range_bool %s_list_2[%u];
extern const uint32_t          %s_list_1[%u];
#    endif
#  endif
