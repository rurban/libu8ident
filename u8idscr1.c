/* libu8ident - Check unicode security guidelines for identifiers.
  ...
  This variant is 2x slower, but 20% smaller.
*/
...
static struct sc *binary_search_range(const uint32_t cp, const char *list,
...
static inline uint8_t sc_search(const uint32_t cp,
                                const struct sc *sc_list_2, const size_t len_2,
                                const struct sc1 *sc_list_1, const size_t len_1)
{
  const struct sc *sc = (struct sc *)binary_search_range(
      cp, (char *)sc_list_2, len_2, sizeof(*sc_list_2));
  if (sc) {
    return sc->scr;
  } else {
    const struct sc1 *sc1 = (struct sc1 *)binary_search_single(
        cp, (char *)sc_list_1, len_1, sizeof(*sc_list_1));
...
static inline bool range_bool_search(const uint32_t cp,
                                     ...
                                     const size_t len) {
  const char *r = (char *)binary_search_range(cp, (char *)list, len, sizeof(*list));
  return r ? true : false;
}

static inline bool single_bool_search(const uint32_t cp,
                                     const uint32_t *list,
                                     const size_t len) {
  const char *r = (char *)binary_search_single(cp, (char *)list, len, sizeof(*list));
  return r ? true : false;
}
...
uint8_t u8ident_get_script(const uint32_t cp) {
  ...
  return sc_search(cp, nonxid_script_list_2, ARRAY_SIZE(nonxid_script_list_2),
                   nonxid_script_list_1, ARRAY_SIZE(nonxid_script_list_1));
#else
  if (s_u8id_options & U8ID_CHECK_XID) // we already checked for allowed
    return sc_search(cp, nonxid_script_list_2, ARRAY_SIZE(nonxid_script_list_2),
                     nonxid_script_list_1, ARRAY_SIZE(nonxid_script_list_1));
  else
    return sc_search(cp, xid_script_list_2, ARRAY_SIZE(xid_script_list_2),
...
const char *u8ident_get_scx(const uint32_t cp) {
  const struct scx *scx = (struct scx *)binary_search_range(
      cp, (char *)scx_list_2, ARRAY_SIZE(scx_list_2), sizeof(*scx_list_2));
  if (scx)
    return scx->list;
  else {
    const struct scx1 *scx1 = (struct scx1 *)binary_search_single(
        cp, (char *)scx_list_1, ARRAY_SIZE(scx_list_1), sizeof(*scx_list_1));
    return scx1 ? scx1->list : NULL;
  }
}
...
bool u8ident_is_allowed(const uint32_t cp) {
  if (!range_bool_search(cp, allowed_id_list_2, ARRAY_SIZE(allowed_id_list_2))) {
    return single_bool_search(cp, allowed_id_list_1, ARRAY_SIZE(allowed_id_list_1));
  } else {
    return true;
  }
}
...
uint16_t u8ident_get_idtypes(const uint32_t cp) {
  const struct range_short *id = (struct range_short *)binary_search_range(
...
#  else
    if (range_bool_search(cp, NFKC_N_list_2, ARRAY_SIZE(NFKC_N_list_2)))
      return true;
    if (single_bool_search(cp, NFKC_N_list_1, ARRAY_SIZE(NFKC_N_list_1)))
      return true;
    if (range_bool_search(cp, NFKC_M_list_2, ARRAY_SIZE(NFKC_M_list_2)))
      return true;
    return single_bool_search(cp, NFKC_M_list_1, ARRAY_SIZE(NFKC_M_list_1));
#  endif
  }
#endif
...
#  else
    if (range_bool_search(cp, NFD_N_list_2, ARRAY_SIZE(NFD_N_list_2)))
      return true;
    return !single_bool_search(cp, NFD_N_list_1, ARRAY_SIZE(NFD_N_list_1));
#  endif
  }
#endif
...
#  else
    if (range_bool_search(cp, NFC_N_list_2, ARRAY_SIZE(NFC_N_list_2)))
      return true;
    if (single_bool_search(cp, NFC_N_list_1, ARRAY_SIZE(NFC_N_list_1)))
      return true;
    if (range_bool_search(cp, NFC_M_list_2, ARRAY_SIZE(NFC_M_list_2)))
      return true;
    return single_bool_search(cp, NFC_M_list_1, ARRAY_SIZE(NFC_M_list_1));
#  endif
  }
#endif
...
#  else
    if (range_bool_search(cp, NFKD_N_list_2, ARRAY_SIZE(NFKD_N_list_2)))
      return true;
    return !single_bool_search(cp, NFKD_N_list_1, ARRAY_SIZE(NFKD_N_list_1));
#  endif
  }
#endif
  return true;
