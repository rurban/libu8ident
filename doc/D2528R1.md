    ...
    Document #: D2538R1
    Date:       2022-03-14 (march 15. deadline.)
...
TODO: add tom, jens, steve as co-authors.
Extended chars? http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2021/p2314r4.html dont mention
...
programmers are, and it's impossible to detect such attacks without
...
* Added wording feedback from the first SSRG discussion,
  and restructure the paragraphs a bit to be less technical, and make it more readable
  to non-Unicode experts.
...
* Honor the Median position in Arabic words,
...
Some actual C++ user-code representing the epsilon transport equation:
...
From visual inspection only you can not decide if the greek letters
here are actual greek, latin or cyrillic.

Even worse is this, which writes an identifier in cyrllic and in latin.
...
_TODO approved: P2071R1 (size argument). java, rust example._

These TR39 changes would fix all of the known security problems with
C++/C identifiers. With C++ it is more severe as declarations are
easily confusable with initializations.
...
Merely checking `confusables.txt` does not properly fix homoglyph
attacks, `confusables.txt` has much more bugs and oddities than
TR31. ASCII has 12 exceptions to be ignored, Greek needs 12
exceptions, and I didn't check any other script.
Following TR39 would be easier to understand, as it is defined by
simple rules, and not a hand-curated and buggy table.
...
* https://cwe.mitre.org/data/definitions/1007.html (gcc CWE-1007 -Whomoglyph patch)
...
Options. (confus and/or mixed/scripts)

Implementation experience (C++ specifically).

Discuss script restrictions (from TR39).

Speed/Size summary.

Why greek should be added. And why it needs to be fixed.

TR would be spec-light.
...
5.2 Script restrictions
-----------------------

More motivation. 

[P1949R7](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2021/p1949r7.html):
"This paper also does not propose excluding any scripts categorically,
regardless of their status as historic or obsolete. Characters from
Anatolian Hieroglyphs would be available for use, to the extent that
anyone wishes to do so."

Recommend to recommended scripts. Excluded and Limited_Use not.
...
And all of the Combining Marks are caught by the NFC requirement in [P1949r7]
...
- **private/scoped**: Another argument would be that all exported names end
